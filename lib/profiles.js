...
let ZCAP_TTL_DEFAULT;
let ZCAP_TTL_PROFILE_ZCAP_INVOCATION_KEY;

bedrock.events.on('bedrock.start', async () => {
  ({
    zcap: {
      ttl: {
        default: ZCAP_TTL_DEFAULT,
        profileCapabilityInvocationKey: ZCAP_TTL_PROFILE_ZCAP_INVOCATION_KEY
      }
    }
...
export async function create({
  ...
  bedrock.events.emit('bedrock-profile.profile.created', {profileId});
...
async function _delegateEdvsZcaps({
  key, keystore, edvs, controller, ttl = ZCAP_TTL_DEFAULT
}) {
  const allZcaps = {};
...
async function _delegateEdvZcaps({
  key, keystore, edv, controller, ttl = ZCAP_TTL_DEFAULT
}) {
...
async function _delegateEdvDocumentsZcap({
  key, edvId, controller, ttl = ZCAP_TTL_DEFAULT
}) {
  return delegate({
...
async function _delegateProfileCapabilityInvocationKeyZcap({
  ...
  const expires = new Date(Date.now() + ZCAP_TTL_PROFILE_ZCAP_INVOCATION_KEY);
  return delegate({
